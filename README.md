...
```bash
# Record for 15 seconds (default) with hot pixel masking
./event_recorder

# Record for 30 seconds (masked recording time)
# Total time = 5s warm-up + 30s recording + 5s cool-down = 40s
./event_recorder -t 30.0
...
The program uses Metavision HAL (no Camera class) and automatically checks for `config/hot_pixels.json` on Linux. The recording runs with a 3-phase protocol:

1. **PHASE 1 (warm-up)**: Normal state — no mask applied. Duration: 5 seconds.
2. **PHASE 2 (masked recording)**: Hot pixel mask is applied via V4L2. Duration: specified by `-t` (default: 15 seconds).
3. **PHASE 3 (cool-down)**: Mask is cleared, returning the sensor to normal state. Duration: 5 seconds.

The `-t` option specifies the **masked recording time** (Phase 2). The total recording time includes the warm-up and cool-down margins:

```
Total time = 5s (warm-up) + <-t duration> + 5s (cool-down)
```
