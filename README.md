...
```bash
# Run recording (records for 90 seconds with 3-phase dynamic masking)
./event_recorder
...
The program automatically checks for `config/hot_pixels.json` on Linux, and runs a 90-second recording session with the following phases:

1. **PHASE 1 (0–30s)**: Normal state (No mask applied).
2. **PHASE 2 (30–60s)**: Automatically applies the hot pixel mask loaded from `config/hot_pixels.json` via V4L2.
3. **PHASE 3 (60–90s)**: Dynamically clears the mask, returning the sensor to normal state.
